...
| Step 4 — LLM 한 줄 코멘트 생성 (★은 코드 레벨) | LLM (Gemini)    | (inline in main.py) |
...
Python 규칙 기반으로 섹션을 빌드하고, LLM은 한 줄 코멘트만 담당한다. ★ 부여는 코드 레벨에서 처리한다:
...
## ⚡ Claude Code 효율 가이드 (토큰 최적화)

**1. 외과적 변경 및 국소적 컨텍스트 (Surgical & Local Context)**

- 수정 요청 시, 관련 함수나 특정 코드 블록만 읽고 수정하라.
- 수정과 무관한 인접 코드나 파일을 분석하기 위해 `ls`나 `cat`을 남발하지 마라.

**2. 산출물 데이터 무시 (Ignore Output Data)**

- `output/` 폴더 내의 실제 JSON/MD 데이터는 분석하지 마라.
- 데이터 구조가 궁금하다면 `CLAUDE.md`의 "산출물 스키마" 섹션만 참고하라.

**3. 스킬 단위 격리 (Skill Isolation)**

- 작업 시 `.claude/skills/` 하위의 특정 스킬 폴더로 범위를 한정하라.
- 오케스트레이터(`main.py`) 수정 시에도 전체를 다시 쓰지 말고 해당 로직만 교체하라.

**4. 선 계획 후 실행 (Plan First)**

- 복잡한 변경은 코딩 전 마크다운 형식으로 '최소한의 변경 계획'을 먼저 제시하고 승인을 받아라.
...
| 실행 환경                   | GitHub Actions cron 2회 —`30 22 * * *` UTC (= JST 07:30 모닝), `30 11 * * *` UTC (= JST 20:30 나이트). `concurrency` 설정으로 동일 워크플로우 중복 실행 방지 |
